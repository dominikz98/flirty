...
/// raw answers per <c>Question.Key</c> and encodes them – like the interactive source – as JSON
...
public sealed class ScriptedAnswerSource : IAnswerSource
{
    ...
    /// No answer was provided for the question's key.
    /// </exception>
    public string GetAnswer(QuestionView question)
    {
        ...
        if (!_answersByQuestionKey.TryGetValue(question.Key, out var raw))
        {
            throw new KeyNotFoundException(
                $"No scripted answer provided for the question '{question.Key}'.");
        }
