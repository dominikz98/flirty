...
/// <param name="ReceivedAt">Point in time at which the handler received the notification.</param>
public sealed record TriggerLogEntry(string DialogKey, Guid SessionId, int AnswerCount, DateTimeOffset ReceivedAt);

/// <summary>
/// Thread-safe in-memory sink for the in-process triggers received by the custom
/// <see cref="DemoDialogCompletedHandler"/>. Registered as a singleton and read by the endpoint
/// <c>GET /demo/triggers</c>, so that the chat UI can make the triggering of the handler visible.
/// </summary>
