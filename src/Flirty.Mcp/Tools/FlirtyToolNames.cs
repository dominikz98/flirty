...
/// declared here in the stage that registers its tool and never ahead of it. Thirty-eight today: the
/// twenty-seven admin tools of stages #126 and #127, the five <c>flirty_session_*</c> of the runtime
/// stage #128, the four <c>flirty_db_*</c> of the database stage #129, <c>flirty_question_type_list</c>
/// from #136 and <c>flirty_placeholder_list</c> from #140. For the same reason the class holds nothing but
/// string literals – the test filters on exactly that, so any other member would silently join the checklist.
/// </para>
/// <para>
/// One of the thirty-eight is <b>conditional</b>: <c>flirty_db_migrate</c> is registered only when the host
...
internal static class FlirtyToolNames
{
    ...
    // ---- Placeholders (1) – FlirtyPlaceholderTools, no MapXxxEndpoints counterpart ---------------

    /// <summary>Lists the message placeholders the host declared with <c>AddPlaceholder</c>.</summary>
    internal const string PlaceholderList = "flirty_placeholder_list";
