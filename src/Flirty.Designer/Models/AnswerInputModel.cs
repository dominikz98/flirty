...
/// <c>"true"</c>/<c>"false"</c>, option values of a multi-choice). The translation into the raw
/// JSON answer value of the engine is done exclusively by the <c>AnswerValueCodec</c> – nothing of it is
/// deliberately rebuilt here.
/// </para>
/// <para>
/// Deviating from the other designer models it is <c>public</c>, for the same reason as
/// <see cref="AnswerChoice"/>: the type is <c>[Parameter]</c> of the component <c>AnswerInput</c>.
/// </para>
/// </remarks>
public sealed class AnswerInputModel
{
    ...
    /// <summary>
    /// Creates the input state from an already given answer – starting point for editing.
    /// </summary>
    /// <param name="type">The answer type of the question.</param>
    /// <param name="value">The stored raw JSON answer value.</param>
    /// <returns>The filled input state.</returns>
    ...
    /// <summary>Sets or removes an option of the multi-choice.</summary>
    ...
    /// domain check deliberately stays with the engine (<c>AnswerValidator</c>), so that the runner shows exactly
    /// the messages a host app would also get.
    /// </summary>
