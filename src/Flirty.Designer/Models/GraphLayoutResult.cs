...
{
    /// <summary>From one layer into the next – the regular case, an arc downward.</summary>
    Forward = 0,
...
/// all questions count as reachable – otherwise the whole graph would be marked as broken, although only one
/// detail is missing.
/// </param>
/// <param name="IsPinned">
/// Whether <see cref="X"/>/<see cref="Y"/> come from a saved position (<c>DialogLayout</c>) and
/// not from the auto-layout. <see cref="Layer"/> and <see cref="Slot"/> still come, even then, from
/// the arrangement – a drag changes the position, not the structure.
/// </param>
...
/// would later make E2E selectors wobble.
/// </remarks>
...
/// The number of edge crossings between adjacent layers after the sorting. Makes the quality
/// of the arrangement measurable, instead of having to assert it.
/// </param>
/// <param name="Width">Total width of the canvas in px.</param>
/// <param name="Height">Total height of the canvas in px.</param>
