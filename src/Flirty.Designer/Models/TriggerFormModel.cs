...
/// <see cref="Name"/> and <see cref="Url"/>. The authoritative type here is the public core type
...
internal sealed class TriggerFormModel
{
    ...
    /// <summary>The point in time in the dialog flow at which the trigger fires.</summary>
    public TriggerScope Scope { get; set; } = TriggerScope.OnDialogCompleted;

    /// <summary>
    /// The question that is listened for on <see cref="TriggerScope.AfterQuestion"/>. Deliberately
    ...
    /// <summary>The optional domain event name (header <c>X-Flirty-Trigger</c>).</summary>
    ...
    /// <summary>
    /// Indicates whether the configuration is edited as raw JSON. Is set by <see cref="From"/>
    /// when the stored JSON cannot be mapped losslessly onto the individual fields.
    /// </summary>
    ...
    /// <summary>
    /// Builds from the input fields the JSON for <see cref="TriggerDefinition.Config"/> and checks it
    ...
    /// silently non-delivering trigger.
    /// </summary>
    ...
    public bool TryBuildConfig(out string json, out string? error)
    {
        ...
        if (UseRawJson)
        {
            ...
            // Take over unchanged – foreign fields are thereby preserved.
    ...
    /// <summary>
    /// Sets the question reference matching the point in time: only <see cref="TriggerScope.AfterQuestion"/> may
    /// carry one (the admin commands reject everything else).
    /// </summary>
    ...
    /// <summary>
    /// Takes over the stored configuration JSON into the individual fields – or falls back to
    /// raw editing if it cannot be mapped losslessly.
    /// </summary>
