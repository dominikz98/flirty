...
/// <summary>
/// Arranges the dialog graph automatically – a "Sugiyama-light": layering via breadth-first search from the
/// entry question, backward edges broken up, crossings reduced via barycenter.
/// </summary>
/// <remarks>
/// <para>
/// <b>Why built in-house:</b> dagre and ELK are JS libraries and have no widespread
/// equivalent in .NET; a Node toolchain is deliberately not wanted in this repo (ADR 0006). The alternative would have been
/// to show the graph unarranged – without saved positions (those first arrive with #102) that is
/// none.
/// </para>
/// <para>
/// <b>Determinism here is no comfort, but a requirement.</b> The same graph must yield the same
/// coordinates, otherwise E2E selectors and screenshots later wobble. This is secured at
/// three places: exclusively <b>lists in fixed order</b> are returned (never a
/// set or a dictionary, whose iteration order is not guaranteed); the sort keys
/// end with a <b>unique</b> ordinal, so they are a total order and do not rely on the stability
/// of <c>OrderBy</c>; and coordinates arise only from integer layer/column values,
/// never from a barycenter – floating-point averages determine the order, not the position.
/// </para>
/// <para>
/// The ordinal of a question comes from <c>(Order, Id)</c> and <b>not</b> from the Guid alone:
/// <c>CreateDialogVersionCommand</c> assigns each question a new Guid on cloning, a Guid-based
/// layout would therefore reshuffle on every new dialog version. <c>Order</c> survives the cloning and
/// is on top of that the order chosen by the author.
/// </para>
/// <para>
/// <b>Without dummy nodes.</b> A full Sugiyama pulls chains of placeholders through
/// skipped layers. They would be needed here only for back-jumps – and those do not belong visually
/// between the nodes anyway, but in a channel at the edge. For the target size of around 30 nodes
...
{
    /// <summary>How often the layers are sorted through for crossing reduction.</summary>
    /// <remarks>
    /// Fixed number instead of stopping on convergence: a convergence criterion on floating-point values would be the
    /// kind of condition that goes differently on another machine.
    /// </remarks>
    private const int SweepCount = 4;

    /// <summary>Corner radius of the back-jump channels in px.</summary>
    private const double CornerRadius = 12;

    /// <summary>Arranges the graph of a dialog.</summary>
    ...
    public static GraphLayoutResult Compute(DialogDetail detail)
    {
        ...
        {
            // The minimum area is no cosmetic: onto an empty dialog the first question is dragged from the
            // palette (#103) – an 80 × 80 px area would be no target.
    ...
    /// <summary>
    /// Reads the positions saved by the author (<c>DialogLayout</c>) from the dialog.
    /// </summary>
    /// <remarks>
    /// <para>
    /// They take effect only at the very end, in <see cref="Render"/>: layering, edge shape, barycenter and
    /// channel assignment stay with the auto-layout. A moved node thereby changes <b>only its
    /// position</b> – never the drawing shape of an edge and never the arrangement of the other nodes. Otherwise
    /// a single drag could resort the whole graph.
    /// </para>
    /// <para>
    /// The determinism promise stays: same input – graph <i>and</i> layout rows – yields
    ...
    private static Dictionary<Guid, (double X, double Y)> Pinned(DialogDetail detail, GraphOrder order)
    {
        ...
        foreach (var entry in detail.Layout)
        {
            ...
            // Negative values are rejected by the command; a hand-written record would otherwise come
            // to lie outside the canvas and would be unreachable.
    ...
    /// later determinism promise.
    /// </summary>
    private static GraphOrder Normalize(DialogDetail detail)
    {
        // (Order, Id): Order is the author order and survives the cloning of a dialog version,
        // the Id resolves the tie that the designer itself never produces.
        ...
        // Only drawable edges. The global Priority sorting from DialogDetail does not suffice as an
        // order: with equal Priority in different source questions it is arbitrary.
    ...
    /// finds shortest paths, for every edge within a component <c>layer[target] ≤ layer[source] + 1</c> holds –
    /// a forward edge therefore never skips a layer.
    /// </summary>
    private static LayerAssignment AssignLayers(DialogDetail detail, GraphOrder order)
    {
        ...
        {
            // Without an entry question there is no reference point for "reachable". Then all questions would be
            // marked red, although only one detail is missing – the finding belongs on the dialog, not on
            // each node. Roots here are the questions that nothing points to.
        ...
        // What is still missing now hangs on no path from the entry question. It comes behind the graph,
        // separated by an empty layer – the band is the visual statement "does not belong to it".
    ...
    // ---- Step 2: break up backward edges ------------------------------------------------------

    /// <summary>
    /// Classifies the edges by their drawing shape. Only <see cref="GraphEdgeShape.Forward"/> edges
    /// between adjacent layers take part in the arrangement – back-jumps are removed from the acyclic edge set,
    /// otherwise the cycle would pull the arrangement together in itself.
    /// </summary>
    ...
    /// <summary>
    /// Resorts the nodes within their layers so that as few edges as possible cross:
    /// each node moves to the mean ("barycenter") of its neighbours in the respective fixed
    /// neighbour layer.
    /// </summary>
        ...
        out int crossings)
    {
        ...
        // Only edges between immediately adjacent layers take part in the arrangement; everything else has, in the
        // dummy-free variant, no defined place in the intermediate layer.
    ...
    /// unique ordinal and is thus a total order – two calls cannot go
    /// differently.
    /// </summary>
        ...
        IReadOnlyDictionary<Guid, int> ordinal)
    {
        ...
        var sorted = layer
            ...
            // Questions of the same loop stay together – otherwise the range frame later spans
    ...
    /// quadratic effort is irrelevant; the value makes the arrangement measurable instead of assertable.
    /// </summary>
    ...
    /// <summary>Assigns to each question the loop within whose range it lies.</summary>
    ...
    // ---- Steps 4 and 5: coordinates and edge course ---------------------------------------------
        ...
        IReadOnlyDictionary<Guid, (double X, double Y)> pinned)
    {
        ...
        for (var layerIndex = 0; layerIndex < slots.Count; layerIndex++)
        {
            ...
            for (var slot = 0; slot < layer.Count; slot++)
            {
                ...
                // The layer is centered. All factors are integer, PitchX is even –
                ...
                // The saved position beats the computed one – and only here, so that it takes the
                // edge routing below along, but has not influenced the arrangement above.
        ...
        // The canvas must also fit moved nodes: the channel of the back-jumps lies to the right
        // of everything that is drawn – otherwise it would run across a widely dragged node.
        ...
        // to drop the third (#103). Both constants are integer – the determinism promise
        // stays untouched.
        var width = Math.Max(
    ...
    /// <summary>
    /// Assigns the channels to the right of the graph to the back-jumps – the longest outermost, so that the
        ...
        double laneX)
    {
        ...
        switch (shape)
        {
            case GraphEdgeShape.Forward:
            {
                ...
                // layers only GapY lie between the nodes, a fixed value would push the
            ...
            default:
            {
                // Back-jumps run in the channel to the right past the graph upwards. Drawn between the
                // nodes they would conceal the flow that the canvas is meant to show.
                var sx = from.X + GraphMetrics.NodeWidth;
                var sy = from.Y + middle + offset;
