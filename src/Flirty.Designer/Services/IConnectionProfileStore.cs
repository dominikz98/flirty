...
/// profile). The storage is deliberately outside the Flirty database, because the profiles are what
/// first establish the connection to that database (chicken/egg).
/// </summary>
internal interface IConnectionProfileStore
{
    ...
    /// <summary>Remembers the given profile as the default (or clears the marker when <c>null</c>).</summary>
