...
4. **Dialog-Versionierung**: Sessions pinnen ihre Dialogversion. Damit das trägt, ist eine **veröffentlichte** Version unveränderlich (Graph-Änderungen → `DialogPublishedException`/409) und Weiterentwicklung läuft über `CreateDialogVersionCommand` (Klon als Entwurf, nächste Versionsnummer); `PublishDialogCommand` zieht die bisher produktive Version zurück. Verworfene Alternativen in [ADR 0005](./adr/0005-unveraenderliche-veroeffentlichte-dialogversion.md).
